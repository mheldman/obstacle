...
class linear_pfas_solver:
    ...
    def lvl_solve(self, lvl, u, b, cycle, diagnostics=None):
        ...
        if lvl < len(self.levels) - 2:
            ...
            if cycle == 'W':
                self.lvl_solve(lvl + 1, coarse_u, coarse_b, cycle,diagnostics=diagnostics)
                self.lvl_solve(lvl + 1, coarse_u, coarse_b, cycle,diagnostics=diagnostics)
                ...
                self.lvl_solve(lvl + 1, coarse_u, coarse_b, 'FV',diagnostics=diagnostics)
                self.lvl_solve(lvl + 1, coarse_u, coarse_b, 'V',diagnostics=diagnostics)

            elif cycle == 'FW':
                self.lvl_solve(lvl + 1, coarse_u, coarse_b, 'FW',diagnostics=diagnostics)
                self.lvl_solve(lvl + 1, coarse_u, coarse_b, 'W',diagnostics=diagnostics)

            elif cycle == 'fmgV':
                coarse_b2 = R.dot(b)
                self.lvl_solve(lvl + 1, coarse_u, coarse_b2, 'fmgV',diagnostics=diagnostics)
                self.lvl_solve(lvl + 1, coarse_u, coarse_b, 'V',diagnostics=diagnostics)
        ...
        if 'show reduced space' in diagnostics:
            ...
            for i in range(0, self.level.my + 2):
                for j in range(0, self.level.mx + 2):
                    k = kk(i, j)
                    Z[j, i] = u[k]
            X = np.linspace(self.level.x1, self.level.x2, self.level.mx + 2)
            Y = np.linspace(self.level.y1, self.level.y2, self.level.my + 2)
            A, B = np.meshgrid(X, Y)
            fig = plt.figure()
            ax = fig.gca(projection='3d')
            surf1 = ax.plot_surface(A, B, Z, color='r', vmin=0.0, vmax=5.1, alpha=.5)
            plt.ioff()
            surf1.show()
    ...
    def solve(self, b, u0=None, cycle='FV', diagnostics=None):

        if u0 is None:
            u0 = np.zeros_like(b)

        u = np.array(u0)
        self.lvl_solve(0, u, b, cycle, diagnostics=diagnostics)
        return u
