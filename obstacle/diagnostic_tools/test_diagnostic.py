...
from grid_transfers_diagnostic import restrict_inj, restrict_fw, interpolate
from multigrid_diagnostic import linear_pfas_solver, level, multigrid_solver
from GS_diagnostic import pgs, gs
import numpy as np
from obstacle_diagnostic import box_obstacle_problem
...
def pfas_test(bounds, f, g, psi, cycle, uexact = None, coarse_mx = 1,\
                                coarse_my = 1, min_num_levels = 5, max_num_levels = None, u0=None):
    ...
    for i in range(min_num_levels, max_num_levels):
        ...
        num_levels = i
        mx, my = coarse_mx, coarse_my
        lvl = level(mx, my, poisson2d, restrict_inj, interpolate, x1, x2, y1, y2, bndry_pts=None)  # currently uses only injection operator
        levels.append(lvl)
        ...
        pfas_solver = linear_pfas_solver(levels, coarse_mx, coarse_my, pgs, diagnostics=('smoothing factors', 'show residuals', 'geometric convergence factor'))
        print(pfas_solver)
        tstart = time()
        print(obstacle_problem.F[[obstacle_problem.F != 0]])
        if u0 is None:
            U = obstacle_problem.solve(pfas_solver.solve, obstacle_problem.F, cycle=cycle, maxiters=25, u0 = obstacle_problem.U)
        else:
            U = obstacle_problem.solve(pfas_solver.solve, obstacle_problem.F, cycle=cycle, maxiters=25,
                                       u0=u0 - obstacle_problem.P)
        timex = time() - tstart
        N = (mx + 2) * (my + 2)
        if uexact is not None:
            Uexact = np.zeros(N)
...
def rsp_test(bounds, f, g, psi, mx, my, linear_solver, tol = 1e-2, maxiters = 100, uexact = None, u0=None,
             diagnostics=('smoothing factors', 'show residuals','geometric convergence factor')):
    ...
    rsp_solver = rspmethod_lcp_solver(obstacle_p.A, obstacle_p.F, maxiters = maxiters,\
                                  tol = tol, diagnostics=diagnostics)

    tstart = time()
    if u0 is None:
        U = obstacle_p.solve(rsp_solver.solve, linear_solver = linear_solver, bounds=bounds, mx=mx, my=my)
    else:
        U = obstacle_p.solve(rsp_solver.solve, linear_solver=linear_solver, bounds=bounds, mx=mx, my=my,
                                 init_iterate=u0)
    timex = time() - tstart
