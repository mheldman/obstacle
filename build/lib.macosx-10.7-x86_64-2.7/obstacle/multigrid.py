...
class linear_pfas_solver:
    ...
    def solve(self, b, u0=None, cycle='FV', tol=1e-8, maxiters=400, smoothing_iters=1):
        ...
        if u0 is None or u0 == 'zero':
            ...
            print('option for initial guess not recognized' + '\n' + \
                  'acceptable types are numpy arrays and the strings ["gmg", "gs", "spsolve", "zero"]' + '\n' + 'using u0 = zeros_like(b) or u0 = b' + '\n')
            if len(self.levels[0].bndry_pts) > 0:
                u0 = np.zeros_like(b)
                u0[self.levels[0].bndry_pts] = b[self.levels[0].bndry_pts]
            else:
        ...
        while residuals[-1] / residuals[0] > tol and z < maxiters:
            ...
            if z != 0 and (active_set_new == active_set_old or (residuals[-1] / residuals[0]) ** (
                1.0 / (len(residuals) - 1.0)) > .75):  # z!=0 and residuals[-1]/residuals[-2] > .5:
                ...
                print('geometric convergence factor PFAS: ' + str(
                    (residuals[-1] / residuals[0]) ** (1.0 / (len(residuals) - 1.0))) + '\n')
                break
